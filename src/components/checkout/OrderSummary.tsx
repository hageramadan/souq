...
export default function OrderSummary({ 
  ...
  // ✅ استخدام رمز العملة الممرر أو القيمة الافتراضية
  const currency = currencySymbol || t('checkout.currency') || "ج.م";
  ...
  const getDeliveryFeeDisplay = () => {
    ...
    if (deliveryFee === undefined || deliveryFee === null) return "--";
    if (deliveryFee === 0) return "--";
    return `${currency} ${deliveryFee.toFixed(2)}`;
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm md:sticky top-20 mb-4 md:mb-0">
      ...
      <div className="space-y-3 pt-3 border-t border-gray-100">
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">{t('checkout.subtotal')}</span>
          <span className="text-gray-800">{currency} {subtotal?.toFixed(2) || "0.00"}</span>
        </div>
        ...
        {discount > 0 && (
          <div className="flex justify-between text-sm text-gray-600">
            ...
            <span className="text-[#23A6F0]">-{currency} {discount.toFixed(2)}</span>
          </div>
        )}
        ...
        {couponDiscount > 0 && couponCode && (
          <div className="flex justify-between text-sm text-gray-600">
            ...
            <span className="text-[#23A6F0]">-{currency} {couponDiscount.toFixed(2)}</span>
          </div>
        )}
