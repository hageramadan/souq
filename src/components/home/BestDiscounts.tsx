...
export function BestDiscounts({ onLoad }: BestDiscountsProps) {
  ...
  // ✅ استدعاء onLoad بعد تحميل البيانات - المكان الصحيح
  useEffect(() => {
    if (!isInitialLoading && onLoad && !hasCalledOnLoad.current) {
      hasCalledOnLoad.current = true;
      onLoad();
    }
  }, [isInitialLoading, onLoad]);
  ...
  // ✅ حالات التحميل - من غير ما نادي onLoad
  if (isInitialLoading) {
    return (
  ...
  // ✅ حالة الخطأ - من غير ما نادي onLoad (الـ useEffect هو اللي هينادي)
  if (error && products.length === 0) {
    return null;
  }

  // ✅ حالة عدم وجود منتجات - من غير ما نادي onLoad (الـ useEffect هو اللي هينادي)
  if (products.length === 0) {
    return null;
  }
